...
// Route configuration: Maps pageId to URL path, hash fallback, and page title
const ROUTE_CONFIG = {
  'page-home': {
    path: '/',
    hash: 'home',
    title: 'Voyage Viste - Official Airline Tickets, Airfare Deals & Travel Services'
  },
  'page-about': {
    path: '/about',
    hash: 'about',
    title: 'About Us - Voyage Viste'
  },
  'page-contact': {
    path: '/contact',
    hash: 'contact',
    title: 'Contact Support Desk - Voyage Viste'
  },
  'page-privacy': {
    path: '/privacy-policy',
    hash: 'privacy-policy',
    title: 'Privacy Policy - Voyage Viste'
  },
  'page-terms': {
    path: '/terms-and-conditions',
    hash: 'terms-and-conditions',
    title: 'Terms and Conditions - Voyage Viste'
  },
  'page-refund': {
    path: '/refund-policy',
    hash: 'refund-policy',
    title: 'Refund Policy - Voyage Viste'
  },
  'page-disclaimer': {
    path: '/disclaimer',
    hash: 'disclaimer',
    title: 'Disclaimer - Voyage Viste'
  },
  'page-results': {
    path: '/flights',
    hash: 'flights',
    title: 'Flight Deals & Search Results - Voyage Viste'
  },
  'page-checkout': {
    path: '/checkout',
    hash: 'checkout',
    title: 'Secure Flight Checkout - Voyage Viste'
  },
  'page-confirmation': {
    path: '/confirmation',
    hash: 'confirmation',
    title: 'Booking Confirmation - Voyage Viste'
  }
};
...
const PATH_MAP = {
  ...
  '/privacy-policy.html': 'page-privacy',
  '/terms': 'page-terms',
  '/terms-and-conditions': 'page-terms',
  '/terms.html': 'page-terms',
  '/terms-and-conditions.html': 'page-terms',
  '/refund': 'page-refund',
  '/refund-policy': 'page-refund',
  '/refund.html': 'page-refund',
  '/refund-policy.html': 'page-refund',
  '/disclaimer': 'page-disclaimer',
  '/disclaimer.html': 'page-disclaimer',
  '/flights': 'page-results',
  '/flights-results': 'page-results',
  '/results': 'page-results',
  '/flights.html': 'page-results',
...
// Funnel Navigation Controller: Manages pages, clean URLs, dynamic header, minimal footer, and adaptive mobile nav
window.navigateToFunnel = function(pageId, pushState = true) {
  ...
  const route = ROUTE_CONFIG[pageId];
  if (route) {
      ...
      if (isFileProtocol) {
        if (pageId === 'page-home') {
          if (window.location.hash) {
            history.replaceState({ pageId }, route.title, window.location.pathname);
          }
        } else {
        ...
        if (pushState) {
          if (window.location.pathname !== targetPath) {
            window.history.pushState({ pageId }, route.title, targetPath);
          }
        } else {
          if (window.location.pathname !== targetPath) {
            window.history.replaceState({ pageId }, route.title, targetPath);
          }
        }
      }
    } catch (e) {
  ...
  if (siteHeader) {
    if (isFunnelStep) {
      ...
      if (pageId === 'page-home') {
        const homeLink = document.querySelector('.nav-menu .nav-item a[href="/"], .nav-menu .nav-item a[onclick*="page-home"]');
        if (homeLink) homeLink.classList.add('active');
      } else if (pageId === 'page-about') {
        const aboutLink = document.querySelector('.nav-menu .nav-item a[href="/about"], .nav-menu .nav-item a[onclick*="page-about"]');
        if (aboutLink) aboutLink.classList.add('active');
      } else if (pageId === 'page-contact') {
        const contactLink = document.querySelector('.nav-menu .nav-item a[href="/contact"], .nav-menu .nav-item a[onclick*="page-contact"]');
...
// URL and History Routing Support (HTML5 History API + Hash fallback)
...
function handleInitialRoute() {
  ...
  window.navigateToFunnel(targetPage, false);
}

window.addEventListener('popstate', function(event) {
  if (event.state && event.state.pageId) {
    window.navigateToFunnel(event.state.pageId, false);
  } else {
    const targetPage = resolveCurrentRoute();
    window.navigateToFunnel(targetPage, false);
  }
