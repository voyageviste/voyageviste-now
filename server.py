...
class VoyaFlightHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Clean URL mapping for Rail landing pages & SPA pages
        clean_path = parsed.path.rstrip('/')
        if clean_path in ('/trains-in-usa', '/amtrak'):
            self.path = '/trains-in-usa.html'
        elif clean_path in ('/auto-train', '/autotrain'):
            self.path = '/auto-train.html'
        elif clean_path in ('/trains-in-canada', '/viarail', '/via-rail'):
            self.path = '/trains-in-canada.html'
        elif clean_path in (
            '', '/home', '/about', '/about-us', '/contact', '/contact-us',
            '/privacy', '/privacy-policy', '/terms', '/terms-and-conditions',
            '/refund', '/refund-policy', '/disclaimer',
            '/flights', '/flights-results', '/results',
            '/checkout', '/confirmation'
        ) or (clean_path.endswith('.html') and clean_path[:-5] in (
            '/about', '/about-us', '/contact', '/contact-us',
            '/privacy', '/privacy-policy', '/terms', '/terms-and-conditions',
            '/refund', '/refund-policy', '/disclaimer',
            '/flights', '/checkout', '/confirmation'
        )):
